...
class SessionServletSpec extends FunSuite with ScalatraSuite with ServletTestDependencies {
  ...
  val s = new SessionServlet(config, database, oppijanTunnistus, verifier, countries, languages, educations, emailSender)
  addServlet(s, "/*")
  ...
  val emailTokenPayload = """{"email":"jussi.vesala@gmail.com"}"""
