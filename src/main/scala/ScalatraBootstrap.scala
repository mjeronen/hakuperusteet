...
class ScalatraBootstrap extends LifeCycle with GlobalExecutionContext {
  ...
  val applicationObject = Tarjonta.init(config)
  ...
  override def init(context: ServletContext) {
    ...
    context mount(new PropertiesServlet(config, countries, languages, educations, applicationObject), "/api/v1/properties")
