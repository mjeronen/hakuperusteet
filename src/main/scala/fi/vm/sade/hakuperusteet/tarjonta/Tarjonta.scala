...
case class ApplicationSystem(hakuOid: String, formUrl: String, maksumuuriKaytossa: Boolean)
case class EnrichedApplicationObject(hakukohdeOid: String, hakuOid: String, name: String, providerName: String, baseEducations: List[String], description: String, as: ApplicationSystem)
object EnrichedApplicationObject {
  def apply(ao: ApplicationObject, as: ApplicationSystem): EnrichedApplicationObject = EnrichedApplicationObject(ao.hakukohdeOid, ao.hakuOid, ao.name, ao.providerName, ao.baseEducations, ao.description, as)
}
...
case class Tarjonta(tarjontaBaseUrl: String) {
  ...
  private def hakuToApplicationSystem(r: Haku) = ApplicationSystem(r.oid, r.hakulomakeUri, r.maksumuuriKaytossa)
}
...
private case class Haku(oid: String, hakulomakeUri: String, maksumuuriKaytossa: Boolean)