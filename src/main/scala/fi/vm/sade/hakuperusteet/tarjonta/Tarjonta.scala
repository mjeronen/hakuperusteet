...
case class ApplicationObject(hakukohdeOid: String, name: String, providerName: String, providerOids: List[String], baseEducations: List[String], description: String)
...
case class Tarjonta(tarjontaBaseUrl: String) {
  ...
  def getApplicationObject(hakukohdeOid: String) = Option(read[Result](urlToString(tarjontaBaseUrl + "/" + hakukohdeOid))).map(_.result)
    .map(r => ApplicationObject(r.oid, r.hakukohteenNimet.kieli_en, r.tarjoajaNimet.en, r.tarjoajaOids, tarjontaUrisToKoodis(r.hakukelpoisuusvaatimusUris), r.lisatiedot.kieli_en)).get
...
private case class Hakukohde(oid: String, tarjoajaNimet: Nimi2, hakukohteenNimet: Nimi, tarjoajaOids: List[String], hakukelpoisuusvaatimusUris: List[String], lisatiedot: Nimi)
